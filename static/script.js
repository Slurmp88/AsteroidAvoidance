...
gameStates[0] = function(){
    ...
    ctx.fillText("Press Left Shift to view High Scores", c.width/2, c.height/2 + 40);
    ctx.restore();
}
...
gameStates[2] = function(){
    ...
    if(score > highScore){
        ...
        var HSinput = document.getElementById("HighScore");
        HSinput.value = score.toString();
        highScore = score;
        ctx.save();
        ctx.font = "30px Arial";
        ctx.fillStyle = "white";
...
gameStates[3] = function()
{
    ...
    {
        while(document.getElementById(`name${i}`).innerHTML != null)
        {
            scores.push([document.getElementById(`name${i}`).innerHTML, Number(document.getElementById(`score${i}`).innerHTML)])
            i++;
            if(document.getElementById(`name${i}`) == null)
            {
                break;
    ...
    //Display
    i = 0;
    while(scores[i] != null && i < 10)
    {
        if(i < 4)
        {
            ctx.font = `${font -= 5}px Arial`
        }
        ctx.fillText("#" + (i + 1) + " " + scores[i][0] + " : " + scores[i][1],  c.width/2, 90 + (40 * i));
        i++;
    }
